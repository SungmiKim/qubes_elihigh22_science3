...
(function (factory) {
  ...
  RoundSlider.prototype = {
    ...
    _setHandleShape: function (preShape) {
      ...
      allHandles.removeClass(prefix + preShape).addClass(prefix + shape);
    ...
    _setHandleColor: function () {
      ...
      if (shape == "dot") {
        allHandles.css({ "border-color": handleColor, background: "" }).children().css("background", handleColor);
      } else {
        allHandles.css({ "border-color": "", background: handleColor });
      }
    },
    ...
    _setRadius: function () {
      ...
      if (o.svgMode) {
        this.svgContainer.height(d).width(d).children("svg").height(d).width(d);
      }
    },
    ...
    _addSeperator: function (pos, cls) {
      ...
      var lineWrap = this._createElement("span.rs-bar rs-transition " + cls)
        .append(line)
        .rsRotate(pos);
    ...
    _getHandleEle: function ($target) {
      if ($target.hasClass("rs-handle")) return $target;

      // this is for the "dot" handleShape, where the handle having the child element also
      var $parent = $target.parent();
      if ($parent.hasClass("rs-handle")) return $parent;
      return null;
    },
    ...
    _raiseBeforeValueChange: function (action, value, angle) {
      ...
      if (isUserAction) {
        ...
        if (!o.snapToStep && angle !== this["_pre_bvc_ang_h" + index]) {
    ...
    _handleKeyDown: function (e) {
      ...
      if (key == 27)
        // if Esc key pressed then hanldes will be focused out
        this._handles().blur();
    ...
    _changeSliderValue: function (value, angle) {
      ...
      if (!this._showRange) {
        ...
        var isValidRange = (activeHandle == 1 && angle <= this._handle2.angle) || (activeHandle == 2 && angle >= this._handle1.angle);
    ...
    _moveHandle: function (angle, value) {
      ...
      this.bar.rsRotate(angle).children().rsRotate(handleAngle);
    ...
    _createSVGElements: function () {
      ...
      this.$linearGradient = this._createSVG("linearGradient");
      this.$stop1 = this._createSVG("stop.stop1");
      this.$stop2 = this._createSVG("stop.stop2");
      this._append(this.$linearGradient, [this.$stop1, this.$stop2]);
      this._append(svgEle, [this.$linearGradient, this.$pathEle, this.$rangeEle, this.$borderEle]);

      this.svgContainer = this._createElement("div.rs-svg-container").append(svgEle).appendTo(this.innerContainer);
    },
    ...
    _isPropsRelatedToSVG: function (property) {
      ...
      var svgRelatedProps = ["radius", "width", "pathWidth", "rangeWidth", "borderWidth", "borderVisibility", "lineCap", "startAngle", "endAngle"];
    ...
    _updateTooltip: function () {
      var o = this.options,
        tooltip = this.tooltip;
      if (tooltip) {
        if (!tooltip.hasClass("rs-hover")) tooltip.html(this._getTooltipValue());
        this._updateTooltipPos();
      }
    ...
    _refreshCircleShape: function () {
      ...
      var otherShapes = ["half-top", "half-bottom", "half-left", "half-right", "quarter-top-left", "quarter-top-right", "quarter-bottom-right", "quarter-bottom-left"];
    ...
    _appendOverlay: function () {
      ...
      if (shape == "pie") this._checkOverlay(".rs-overlay", 270);
        ...
        if (shape == "custom-quarter") this._checkOverlay(".rs-overlay2", this._arcLength);
      }
    },
    ...
    _createSVG: function (tag, attr) {
      ...
      if (tagName == "linearGradient") {
        svgEle.setAttribute("id", "grad1");
        svgEle.setAttribute("x1", "0%");
        svgEle.setAttribute("y1", "0%");
        svgEle.setAttribute("x2", "100%");
        svgEle.setAttribute("y2", "0%");
      }
      if (className == "stop1") {
        svgEle.setAttribute("offset", "0%");
        svgEle.setAttribute("style", "stop-color:rgb(254,216,5);stop-opacity:1");
      }
      if (className == "stop2") {
        svgEle.setAttribute("offset", "100%");
        svgEle.setAttribute("style", "stop-color:rgb(248, 134, 42);stop-opacity:1");
      }
      if (attr) {
  ...
  RoundSlider.prototype._polarToCartesian = function (radius, angleInDegrees) {
    ...
    return [centerXY + radius * Math.cos(angleInRadians), centerXY + radius * Math.sin(angleInRadians)].join(" ");
  };
  ...
  RoundSlider.prototype._drawArc = function (startAngle, endAngle, radius, isReverseDirection) {
    ...
    if (isCircle) {
      ...
      path.push("A", 1, 1, 0, 0, direction, midPoint, "A", 1, 1, 0, 0, direction, endPoint);
    } else {
      path.push("A", radius, radius, 0, largeArcFlag, direction, endPoint);

      if (isInvertedRange) {
        var sliderStartAngle = this._start;
        var sliderStartPoint = this._polarToCartesian(radius, sliderStartAngle);

        path.push("M " + sliderStartPoint, this._drawArc(sliderStartAngle, actualEndAngle, radius));
      }
    }
  ...
  RoundSlider.prototype._drawPath = function (startAngle, endAngle, outerRadius, innerRadius, lineCap) {
    ...
    if (this._isNumber(innerRadius) && innerRadius !== outerRadius) {
      ...
      if (lineCap == "butt") {
        d.push("L " + innerEnd, innerArc, "L " + outerStart, "Z");
      } else if (lineCap == "round") {
        d.push("A 1, 1, 0, 0, 1, " + innerEnd, innerArc, "A 1, 1, 0, 0, 1, " + outerStart);
        ...
        d.push("L " + endSquare_OuterCorner, "L " + endSquare_InnerCorner, "L " + innerEnd, innerArc, "L " + startSquare_InnerCorner, "L " + startSquare_OuterCorner, "L " + outerStart, "Z");
      } else if (lineCap == "none") {
        d.push("M " + innerEnd, innerArc);
      }
    }
    return d.join(" ");
  };
